    ...
    <div className="relative flex min-h-screen w-full flex-col items-center bg-slate-100 pt-[88px] pb-16 lg:flex-row lg:items-start lg:justify-center lg:gap-6">
        ...
        <div className="-mt-16 mb-4 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm font-medium text-slate-500">
              {userData.headline || "Tell your network what's new"}
            </p>
            <p className="text-xs text-slate-400">
              {userData.location || "Add a location"}
            </p>
          </div>
        </div>
        <button
          className="mt-2 flex w-full items-center justify-center gap-2 rounded-full bg-sky-500 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-sky-600"
          onClick={() => setEdit(true)}
        >
      ...
      {uploadPost && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="relative w-[92%] max-w-xl rounded-3xl border border-slate-200 bg-white p-6 shadow-2xl">
            ...
            <textarea
              className={`w-full rounded-2xl border border-slate-200 bg-slate-50/60 p-4 text-base text-slate-700 placeholder:text-slate-400 transition focus:border-sky-300 focus:outline-none focus:ring-2 focus:ring-sky-100 ${
                frontendImage ? "h-36" : "h-52"
              } resize-none`}
            ...
            <div className="mt-6 flex items-center justify-between border-t border-slate-200 pt-4">
              <button
                type="button"
                className="flex items-center gap-2 rounded-full border border-slate-200 bg-slate-50 px-4 py-2 text-sm font-semibold text-slate-600 transition hover:border-sky-200 hover:bg-sky-50"
              ...
              <button
                className="rounded-full bg-sky-500 px-6 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-sky-600 disabled:cursor-not-allowed disabled:bg-slate-300"
                disabled={posting}
      ...
      <div className="w-full max-w-2xl space-y-6 px-4 lg:px-0">
        <div className="flex w-full items-center gap-3 rounded-2xl border border-slate-200 bg-white/90 p-4 shadow-lg">
          ...
          <button
            className="flex h-12 flex-1 items-center rounded-full border border-slate-200 bg-slate-50 px-5 text-left text-sm font-semibold text-slate-500 transition hover:border-sky-200 hover:bg-sky-50"
      ...
      <div className="hidden w-full max-w-[320px] flex-col gap-4 rounded-2xl border border-slate-200 bg-white/90 p-6 shadow-lg lg:flex">
          ...
          <div className="flex flex-col gap-3">
            {suggestedUser.map((su) => (
              <button
                ...
                className="flex items-center gap-3 rounded-xl border border-slate-200 bg-slate-50/60 p-3 text-left transition hover:border-sky-200 hover:bg-sky-50"
              ...
              >
                ...
                <div>
                  ...
                  <div className="text-xs font-medium text-slate-500">
                    {su?.headline ?? "Let them know what you do"}
                  </div>
                </div>
              </button>
            ))}
        ...
        {suggestedUser.length === 0 && (
          <div className="rounded-xl border border-dashed border-slate-200 bg-slate-50/60 p-4 text-center text-sm font-medium text-slate-500">
